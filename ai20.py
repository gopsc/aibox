    ...
    def _load_strings(self):
        ...
        self._strings = {
            'zh': {
                ...
                # 新增提示词
                'no_timestamp_prompt': "【重要输出规则】在输出任何内容时，都不要添加时间戳前缀。直接输出纯文本内容。",
            },
            'en': {
                # Welcome messages
                ...
                # New prompts
                'no_timestamp_prompt': "【Important Output Rule】Do not add timestamp prefixes to any output. Output pure text content directly.",
            }
        }
...
class PromptManager:
    ...
    def _create_default_prompts(self):
        """创建默认提示词文件 - 添加禁止输出时间戳的提示词"""
        default_prompts = [
            {
                'filename': '00_no_timestamp.txt',
                'content': i18n.get('no_timestamp_prompt')
            },
            {
                'filename': '01_identity.txt',
...
- 使用 `save_memory_and_end_conversation` 工具完成"""
            ...
            {
                'filename': '04_memo_capability.txt',
                'content': """【备忘录能力】
- 你可以创建备忘录提醒用户和自己
- 支持查看、完成、删除备忘录
- 定期检查备忘录是否有需要处理的事项"""
            ...
            {
                'filename': '05_file_operations.txt',
                'content': """【文件操作能力】
- write_file: 写入文件
- read_file: 读取文件
- list_files: 列出目录"""
            ...
            {
                'filename': '06_web_access.txt',
                'content': """【网页访问能力】
- web: 获取网页内容、搜索关键词、下载文件"""
            },
            {
                'filename': '07_memory_management.txt',
                'content': """【记忆管理能力】
...
- memory delete: 删除无用记忆"""
            ...
            {
                'filename': '08_extension_skills.txt',
                'content': """【多种多样的扩展能力】
- 根据工具介绍，使用json格式传递参数
- 优先使用扩展工具"""
            },
            {
                'filename': '09_language_instruction.txt',
                'content': """【语言说明】
...
- 用户可能会要求你用特定语言回复，请遵循用户的指示"""
            ...
            {
                'filename': '10_conclusion.txt',
                'content': """记住：你是智能助手，要主动帮助用户解决问题！"""
            },
            {
                'filename': '11_task_handling.txt',
                'content': """【任务处理规则】
...
当前时间: {datetime.now().strftime(constants.DATETIME_FORMAT)}"""
    ...
    def _add_message(self, role: str, content: str):
        ...
        if role == "assistant":
            # 修复：只添加一次前缀，并且确保内容中没有重复的时间戳
            # 先清理内容中可能已经存在的时间戳
            clean_content = re.sub(r'^\[.*? @ .*?\]\s*', '', content)
            formatted_content = f"[{self.name} @ {timestamp}] {clean_content}"
        elif role == "user":
    ...
    def think_and_respond(self, input_text: str, output_callback: Callable = None) -> Optional[str]:
        ...
        try:
                    ...
                    if line.startswith('data: '):
                        ...
                        try:
                            ...
                            if 'content' in delta and delta['content']:
                                ...
                                # 修改：确保发送的内容是干净的，没有时间戳
                                # DeepSeek API 返回的内容本身不应该有时间戳，但为了安全，还是清理一下
                                clean_content = re.sub(r'^\[.*? @ .*?\]\s*', '', content)
                                chunk_buffer += clean_content
            ...
            if current_tool_calls:
                ...
                # 添加助手消息（不含时间戳前缀，因为 _add_message 会添加）
                assistant_msg_content = full_response if full_response else None
                if assistant_msg_content:
                    # 清理可能已经存在的时间戳
                    clean_content = re.sub(r'^\[.*? @ .*?\]\s*', '', assistant_msg_content)
                    self._add_message("assistant", clean_content)
                    if self.history_manager:
                        self.history_manager.add_message("assistant", clean_content, self.name)
                else:
                    # 如果没有内容，也需要记录空消息
                ...
                # 修改：清理可能已经存在的时间戳
                clean_response = re.sub(r'^\[.*? @ .*?\]\s*', '', full_response)
                self._add_message("assistant", clean_response)
                if self.history_manager:
                    self.history_manager.add_message("assistant", clean_response, self.name)
                
                # 发送完成消息
                if output_callback:
                    output_callback("complete", clean_response)
            
            return clean_response if full_response else ""
...
class WebSocketHandler:
    ...
    async def handle_user_message(self, websocket, data):
        ...
        # 定义输出回调函数 - 实时发送，确保内容没有时间戳
        def output_callback(msg_type, content):
            """AI输出的回调函数 - 实时发送每个chunk，确保内容没有时间戳"""
            ...
            async def send_msg():
                try:
                    await websocket.send(json.dumps({
                        "type": msg_type,
                        "content": clean_content  # 内容已经是干净的，没有时间戳
                    }, ensure_ascii=False))
